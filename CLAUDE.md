...
- **Hearts drain in:** BikeDodgeScene (crashes / bag hits), BeeHiveScene (bee stings — synced back to `HeartsManager` via `remainingHearts`), GameScene (enemy bite), and MopBucketChaseScene (every obstacle crash, via `loseHeart()`). Score-based games (cornhole, baseball, beachball) don't drain the universal count.
- **Hearts refill on:** cold app launch (`LoadingScene.didMove`), world-map game over (`GameScene.triggerGameOver`), in-game pickups (`PickupData.heart` in bike race), and on **replay-after-loss** in any heart-draining game (`BikeDodgeScene.resetGame`, `BeeHiveScene.restartGame`, `MopBucketChaseScene.resetForReplay`). A 0-hearts guard on entry into the bike race, beehive, and mop chase (both from world and picker paths) also refills, so players can't enter a heart-draining game with 0 hearts.
...
**Course** (`buildCourse`) — seeded fresh every run/replay. Static/zone hazards use the `Obstacle` struct (`kind`, `dist`, `length`, `lanes`, `triggered`); dynamic hazards have their own structs driven manually off `raceClock` in `update()` (pause-safe, no SKAction state). Scripted majors at fixed fractions of `hallLength`:
- **Wax zones** ×3 (14/44/78%) — a stroke landing inside slips (×0.60, once per zone); handled in `stroke()`, not `updateObstacles()`.
- **Limbo mop bars** ×3 (22/52/86%) — span 2 lanes, random open outer lane; hitting the bar ×0.45.
- **Spray-bottle kids** ×2 (30/68%) — mist the whole screen for 2 s (`mistOverlay`, full-screen translucent sprite).
- **Puddles** ×3 (37/60/92%) — a stroke *landing* inside gets ×1.6 "SUPER STROKE!", reusable.
- **Swinging doors** ×4 (18/40.5/63/82%, alternating walls) — `SwingDoor` panels hinge on the wall and swing into the outer lane on a sine cycle (`openness > 0.6` = blocking); getting slammed ×0.45.
- **Hall monitors** ×3 (26/56/74%) — `HallMonitor` kids (orange shirt + sash) pace the full hall width on a sine; contact ×0.40 ("NO ROLLING IN THE HALL!").
- **High-five kids** ×5 (12/33.5/48/66.5/88%, alternating walls) — pass in their wall lane for `+W×0.30` speed ("HIGH FIVE!", success haptic).
Random single-lane fillers (kids ×0.35, wet-floor signs ×0.50) every ~0.85–1.35 W, skipping ±0.4 W around majors and stopping with `W×0.6` clearance before `billyWaitDist` so nothing spawns on top of him. Point collisions require lane match (`jackLane`) within 16 pts of `dist`. Every obstacle hit (kid/sign/limbo via `crashInto`, wax, door, monitor, binder) calls `loseHeart()` in addition to its existing speed penalty.

**Hearts** — `heartSprites` (5, `HeartsManager.shared.maxHearts`) render in the top ribbon center (♥/♡, `refreshHearts()`); synced from `HeartsManager.shared` on scene load (0-hearts guard refills, matching the bike race/beehive convention) and refilled on replay-after-loss (`if !didWin { HeartsManager.shared.refill() }` in `resetForReplay`). See **Hearts System** above — this scene is now a heart-draining game.

**Billy** — stands center-lane at `billyWaitDist`, idle-bouncing in place (`"idleBounce"` action) with a crossed-arms sway; he never moves during `.racing`. Once Jack is within `H×1.6` of him, he **hurls his binder** back down Jack's current lane every 3.0–5.5 s (`updateBillyTaunt` → `billyThrowsBinder` → `BillyProjectile`, travels at `W×0.85`; hit = ×0.45 "BEANED BY BILLY!" + `loseHeart()`). HUD center now shows the hearts row instead of a gap readout; the timer (`timeLabel`, blue, turns red at ≤5 s) sits right-aligned near the close button.
...
**Losing** — either the 60 s clock expires before Jack reaches `finaleZoneStart` (`billyLeaves()` — Billy shrugs, bolts up-screen through the doors and fades, "TIME'S UP!") or hearts hit zero from crashing too much (`loseHeart()` → "TOO BANGED UP!"). Both show PLAY AGAIN / EXIT with a `loseReason`-specific hint.
