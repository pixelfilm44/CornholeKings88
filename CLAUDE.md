...
- `SettingsScene` — settings screen (launched from SETTINGS); includes a "reset tutorials" action backed by `TutorialManager.resetAll()`, and a "reset story" action backed by `StoryManager.reset()`. The story reset requires a deliberate second tap (label reads "TAP AGAIN?" for 3 s, then reverts) since it erases all narrative progress, unlike the tutorials reset which fires immediately.
...
|----------|------|---------------|-----------|---------------|
| Tim | `.tom` | TIM | 11 | Baseline AI, moderate accuracy. Random per-round chance to trigger "Tim's Fart" (drifting, semi-transparent green fog over the board; indicator speed unchanged) |
...
**Hangers (partial rest states)** — a bag no longer resolves to a binary on-board / off-board, in-hole / not. `boardZone(_:)` classifies a resting bag as `.on` / `.edgeHang` / `.off`, and `updateHangerState(_:)` (run every frame for every bag from `update()`, including settled ones) derives two states:

| State | Flag | Worth | How it changes |
|-------|------|-------|----------------|
| Balanced on the cup's lip (`holeRadius` → `holeRadius + holeLipBand`, band = `holeRadius * 0.50`) | `isHoleHanger` | **0 pts** — neither in the hole nor flat on the board | A later bag shoving it holeward tips it in for the full 3 (6 golden). Shoved outward → it becomes a normal 1-pt board bag |
| Centre past the visible board rail but within `edgeHangBand` (`min(11, boardHalfW * 0.11)`) | `isEdgeHanger` | **1 pt** (a normal board bag) | A solid hit carries it past the point of balance → falls to the grass for 0 |

- `checkIsOnBoard(_:)` is now `boardZone(_:) != .off`, so edge hangers count as board bags everywhere (scoring, bomb/fire triggers, Jenny/Chuck AI targeting). Hole hangers are excluded from board points explicitly in both `calculateRoundScore()` and `calculateCathyRoundScore()`.
- **Earned special bags never hole-hang** (`MiniGameBag.isSpecialBag` — honey/bomb/magic/fire/cannonball): their value is a guaranteed effect, so they score their normal 1/3 instead of gambling on a nudge. They *can* edge-hang, which only ever gains them a point. Golden bags hang normally — the 6-pt tip-in is the payoff for the risk.
- **The lip is a funnel, not flat wood.** A disturbed hole hanger (`isHoleHanger && hangAnnounced && isMoving`) gets a `0.22`/frame acceleration toward `holeCenter` plus a `0.72`/frame perch drag. Without it, a hanger perched *past* the cup is shoved further away by an incoming bag and slides to the far rail instead of dropping in — the collision system has no idea the wood under it slopes. The drag also bounds how far a bumped hanger travels, while a hard enough direct hit still carries it clean out of the band; that gradient (gentle nudge → drops in, hard hit → knocked clear) is the risk dial.
- **Tip-in rule** — gated on `hangAnnounced` (the bag actually came to rest on the lip), then any velocity with an inward component `> 0.20` captures it; sets `wasPushedIn` for the "PUSHED IN!" callout. A bag still skimming across the band falls through to the normal distance/sweep test. The funnel pull above is what usually drives the inward component past that threshold.
- **Lip drag** — a bag in the `.edgeHang` zone gets `vx/vy *= 0.62` per frame (half of it is unsupported), so moderate slides catch on the rail while a hard hit still carries clean off. Honey bags are exempt (they stick on contact).
- **Rest zones are anchored to the *drawn* board.** `boardContainer` is drawn at `setScale(0.90)`, so `boardRestHalfW/H` use `boardHalfW/H * 0.90` — the visible rail — not the old `0.95` hit-zone fudge. Anchoring anywhere else makes a hanging bag straddle empty grass instead of the wood.
- **Feedback** — the hanging pose is the persistent readout, applied in `updateBagDeform` as *visual offsets only* (`bx`/`by` never move, so collisions and scoring stay honest): a spring-eased dip toward the drop (`hangLean`), a slow teeter, and a shrink part-way toward the 0.75 off-board ground scale. Edge hangers additionally get the four cues that stop them reading as airborne — a screen-Y **height drop** (the same axis `bz * 0.50` rides on), a **solid ground shadow** tucked just beneath them (a faded shadow is what made them look like they were hovering), a direction-aware **roll** (`-hangDirX`) so the outer edge dips the correct way, and a **droop warp** (`makeEdgeDroopGrid`) that foreshortens the unsupported half back toward the bag's centre and slides it down-screen so the bag bends over the rail. Hangers depth-sort as ordinary board bags — **do not** push a far-rail hanger behind the board (`zPosition` below the board's 5). It was tried and reads wrong in this projection: opaque wood hides the supported half, so the bag looks like it's lying on the grass behind the board rather than perched on its edge. The droop grid only takes the mesh once the Tier-3 drape spring is at rest; the two never fight over `warpGeometry`. Plus a pulsing gold ring at the cup while any lip hanger exists (`refreshHoleLipRing()`), a one-shot `showBagTag` callout on settle (spelled out once per match — "HANGER = 0 PTS" / "EDGE HANG = 1 PT" — then terse), and a tumble + "OFF THE EDGE!" on knock-off.
- **AI** — `aiLipTarget()` gives every opponent a 55% chance to spend a throw converting its own settled lip hanger, but only when a throw arriving from the throw line would carry it *toward* the cup. The Fairy Queen (drops from above), CathyX (the hole is a penalty for her), and Grandpa (boom-or-bust, never settles) sit it out.
...
**Mid-match special-bag nudge** — `specialBagNudgeIfNeeded()` fires at most once per match, checked in `concludeRound()` right after a non-final round: if the player is behind on score, has no bag currently armed, and holds at least one unused special bag (checked fire → magic → bomb → cannonball → honey), it shows a one-time banner ("🔥 FIRE BAG READY — TAP TO EQUIP", etc.) via `showStreakBanner`, timed to appear after the round-result banner clears (so hints never stack) and before the next round starts (`startRound()` waits an extra ~1.4 s to make room for it). This is the in-match complement to the post-loss Fairy Queen hint above — it reaches the player before the frustration of a loss, not after.
...
- **Hearts drain in:** BikeDodgeScene (crashes / bag hits), BeeHiveScene (bee stings — synced back to `HeartsManager` via `remainingHearts`), and GameScene (enemy bite). Score-based games (cornhole, baseball, beachball) don't drain the universal count. MopBucketChaseScene does **not** drain hearts — see **Mop Bucket Chase** below, every obstacle crash there costs speed instead.
...
**Entry points:**
- `MiniGamePickerScene` → `"wellflinger"` card (`onComplete = { _ in }`). Free-play from the menu isn't tied to real inventory, so it explicitly sets `s.availableBags = 5` — a generous allowance to learn the shaft, independent of what the player is actually carrying in the world.
- World trigger — any tileset whose name contains `"well"` (`Well.tsx`/`well.png`). `extractWellPositions(from:)` collects tile centers into `wellPositions`; A-press within range checks `inventory.counts[.bag, default: 0] > 0` first — with zero bean bags it shows a hint banner ("You need bean bags\nto throw down\nthe well.") instead of launching. Otherwise `openWellFlinger()` sets `awardsRewards = true` and passes the player's actual `availableBags`/`availableFireBags` counts from inventory — the real-world session is bounded by what they've actually collected, unlike the menu's flat 5. Win awards 3 fire bags (same reward table as the picker path).
...
**Kick resolution** (`resolveKick()`): timing quality falls off linearly over `whiffWindow` (base 64 pt from plate; beyond = "WHIFF!"). `power = 0.30 + 0.70 × (0.55·timing + 0.45·runSpeed)`; a stalled kicker (`runSpeedFactor < 0.05`) has power capped below `hitPowerThreshold`, so a standing kick can never be a hit ("NO LEGS — EASY OUT!"). Timing error skews the direction (early pulls, late pushes). Outcome decided at the moment of the kick, then animated:
- `|angle| > foulAngle (base ±43°)` → **FOUL** (failed try)
- `power < hitPowerThreshold (base 0.62)` → **FIELDED — OUT** (nearest defender chases the projected stall point)
...
**Forgiveness ramp** — this is a story beat, not a skill gate, so nobody should get stuck in the dream. `whiffWindow`, `foulAngle`, and `hitPowerThreshold` are computed properties keyed off `currentTry` (1-indexed) instead of fixed constants: try 1 uses the base values above; try 2 widens the whiff window ×1.35, opens the foul lines ×1.15, and drops the hit threshold by 0.08; try 3 widens further (×1.7 / ×1.3 / −0.16). The player still has to fail once or twice for drama, but a later try is meaningfully easier to connect on rather than an identical roll of the dice.
...
`MopBucketChaseScene` is the Chapter 3 chase: Billy punches Jack and bolts down a hallway, then stops to wait by the far doors, bouncing in place. Jack rows after him — one foot in a rolling mop bucket. **Overhead vertical-scrolling** chase (same top-down camera as the bike race), center-origin, manual physics in `update()` (dt-based, so pause needs no clock shifting).

**Goal: a timed run, not a race.** Billy is stationary (`billyWaitDist`, near the far end) for the whole level — he never moves during `.racing`. The player has `raceTimeLimit = 60 s` (`raceClock`) to reach him; reaching `finaleZoneStart` (`billyWaitDist - W×0.15`) before the clock expires triggers the crash finale (the win). Letting the clock run out calls `billyLeaves()` — Billy shrugs and bolts through the doors — for the only loss condition ("TIME'S UP!"). Crashes don't touch the universal hearts pool at all — every obstacle hit only costs `jackSpeed` (see **Course** below), so the clock is the sole thing standing between the player and Billy. This game does **not** use `HeartsManager`.

**Character scale** — every character built via `makeTopKid` (obstacle kids, spray/high-five kids, limbo kids, Billy, Becky, onlookers, flying Jack) is baked to `characterScale = 2.0` inside the function (`scaled: true` default). Jack's bucket rig is the one exception: `buildJack` builds the body with `scaled: false` and instead calls `jack.setScale(characterScale)` on the whole container (bucket + body + mop together), so nothing gets doubled twice. Any **absolute** `.scale(to:)`/`.scaleY(to:)` action applied later to an already-2×-scaled node must multiply its target by `characterScale` (done for Becky's flinch + knocked-flat recoil and flying Jack's mid-air bounce) — the standalone crash-cutscene bucket/mop/shadow are simpler: their literal geometry is doubled directly instead, since the bucket's tip-over sequence already uses absolute `scaleY` values that would conflict with a baked-in `.setScale`. Detached props (Becky's books, reparented from her to `scrollLayer` on impact) get `.setScale(characterScale)` explicitly since they lose her inherited transform the moment they're reparented. Obstacle collision radii were widened to roughly match the bigger silhouettes (point checks now mostly 20–26 pt instead of 14–18 pt).

**Stroke loop** — a single TAP fires `stroke()`: a fixed impulse (`strokeImpulse = W×1.15`) added to `jackSpeed` (capped at `maxSpeed = W×1.7`), plus a plant-and-sweep mop animation. No charge time, no timing window — every tap counts the same, so speed is purely a function of tap rate versus glide friction (`frictionK = 0.55`, exponential decay). A stroke landing inside a puddle zone gets ×1.6 ("SUPER STROKE!"); a stroke landing inside an untriggered wax zone slips instead of propelling Jack (×0.60 speed) — see **Course** below. Speed streaks (`spawnSpeedStreak`) are pure eye-candy above `fastThreshold = W×0.80`, unrelated to scoring.

**Steering** — 3-lane hallway (`hallWidth = W×0.70`); a horizontal swipe ≥ 30 pt shifts `targetLane` one lane (chained swipes within one touch re-anchor `swipeRefX`); `jackX` eases toward the lane center with a lean tilt. Swiping and stroking share the same touch: a swipe sets `touchDidSteer`, and `touchesEnded` fires `stroke()` only when the touch wasn't a steer (a swipe alone doesn't also fire a stroke).

**Course** (`buildCourse`) — seeded fresh every run/replay. Static/zone hazards use the `Obstacle` struct (`kind`, `dist`, `length`, `lanes`, `triggered`); dynamic hazards have their own structs driven manually off `raceClock` in `update()` (pause-safe, no SKAction state). Hazard variety and count were trimmed from an earlier, denser layout — pacing hall monitors were cut entirely (they overlapped functionally with swinging doors: both were "timed lane obstacle you dodge on a cycle") and every surviving type spawns fewer times with wider spacing so each gets a readable introduction instead of chaining crash after crash. Scripted majors at fixed fractions of `hallLength`:
- **Wax zones** ×2 (16/62%) — a stroke landing inside slips (×0.60, once per zone); handled in `stroke()`, not `updateObstacles()`.
- **Limbo mop bars** ×2 (30/74%) — span 2 lanes, random open outer lane; hitting the bar ×0.45.
- **Spray-bottle kids** ×2 (44/86%) — mist the whole screen for 2 s (`mistOverlay`, full-screen translucent sprite).
- **Puddles** ×2 (24/68%) — a stroke *landing* inside gets ×1.6 "SUPER STROKE!", reusable.
- **Swinging doors** ×3 (20/50/80%, alternating walls) — `SwingDoor` panels hinge on the wall and swing into the outer lane on a sine cycle (`openness > 0.6` = blocking); getting slammed ×0.45.
- **High-five kids** ×4 (12/38/58/90%, alternating walls) — pass in their wall lane for `+W×0.30` speed ("HIGH FIVE!", success haptic).
Random single-lane fillers (kids ×0.35, wet-floor signs ×0.50) every ~1.1–1.7 W (widened from the original 0.85–1.35 W for more breathing room), skipping ±0.4 W around majors and stopping with `W×0.6` clearance before `billyWaitDist` so nothing spawns on top of him. Point collisions require lane match (`jackLane`) within 16 pts of `dist`. Every obstacle hit (kid/sign/limbo via `crashInto`, wax, door, binder) only applies its speed penalty — there's no separate heart deduction.

**Billy** — stands center-lane at `billyWaitDist`, idle-bouncing in place (`"idleBounce"` action) with a crossed-arms sway; he never moves during `.racing`. Once Jack is within `H×1.6` of him, he **hurls his binder** back down Jack's current lane every 3.0–5.5 s (`updateBillyTaunt` → `billyThrowsBinder` → `BillyProjectile`, travels at `W×0.85`; hit = ×0.45 "BEANED BY BILLY!"). HUD center shows the countdown timer (`timeLabel`, blue, turns red at ≤5 s, `PressStart2P` 14pt) — the sole fail-condition readout — instead of a hearts row or gap readout.

**Progress tracker** (`buildProgressTracker`) — side-view strip under the top ribbon: tiny side-profile Jack-in-bucket and standing-Billy minis along a floor line toward a door sprite at the far end, mapped by `dist / hallLength`. `trackBilly` stays fixed at `billyWaitDist`'s position throughout the race.

**Ending (canon: Jack never truly catches him)** — reaching `finaleZoneStart` triggers `startFinale()`: input cut, big "GOTCHA—" beat (Jack's premature victory shout), janitor-water ellipse + book-carrying Becky spawn ahead, Billy's idle bounce stops and he starts peeling off + fleeing (`billyFleeSpeed`) toward the doors. Hitting the water fires a white flash and drops `finaleTimeScale` to 0.40 (slow-mo applied to the manual physics only; it eases back to 1.0 at 0.7/s), with the bucket spinning, Jack fishtailing, and skid marks trailing. Becky shrieks "EEEK!" when Jack is within `W×0.5`. At `beckyDist − 20`, `crashIntoBecky()` swaps the screen-anchored Jack for world-anchored crash actors: the bucket **ricochets off the lockers** (clang + sparks + medium haptic) before bouncing back and tipping over, water sloshes out in expanding waves, the mop cartwheels aside, and Jack launches up-and-over (growing shadow, ×1.6 scale arc, 2.5 spins) into Becky. `impactMoment()`: white flash, shockwave ring, dust cloud, two-axis screen shake, Becky knocked flat, books/papers/pencils exploding outward, 10 impact stars, Jack bouncing to a dazed rest **on top of her** with floating hearts, "CRASH!" — then Billy stops to point and laugh ("HA HA!" + mocking hops) before bolting out the doors, three onlooker kids waddle in to gawk ("OOOOH!"), and the outro toasts land: "...HI, BECKY." / "GET OFF ME, JACK!!" / "JANITOR TO THE MAIN HALL." → win modal "MADE IT... TOO FAST / RIGHT INTO BECKY" (PLAY AGAIN / CONTINUE) at ~4.7 s.

**Losing** — the 60 s clock expiring before Jack reaches `finaleZoneStart` is the only way to lose (`billyLeaves()` — Billy shrugs, bolts up-screen through the doors and fades, "TIME'S UP!"). Shows PLAY AGAIN / EXIT with a hint reminding the player crashes only cost speed, not the clock, so tapping through a crash is always worth it.

**World layout** — the floor/locker background is two screen-anchored texture sprites (`makeHallTexture`) scrolled like the bike-race road; gameplay entities (obstacles, zones, Billy, Becky, doors, floor section labels "GYM HALL/CAFETERIA/TROPHY HALL/MAIN HALL") live in `scrollLayer` where world-Y = hall distance; `layoutWorld()` sets `scrollLayer.position.y = jackScreenY - jackDist` each frame so screen-anchored Jack stays fixed at `y = -H×0.22`. Billy's world-Y is left untouched by `layoutWorld()` while racing (he's static and free to run his own idle bounce) and is only driven directly during the finale/timeout flee. `resetForReplay()` tears down and rebuilds the whole world — fresh random course each replay.
...
`BeeHiveScene` tracks a global **win** count in `UserDefaults` key `beeHiveWinCount_v1` (a new key — replaces the old `beeHiveFightCount_v1`, which incremented on every fight regardless of outcome and is no longer read, so existing installs don't inherit an inflated difficulty from a mix of wins and losses). `dismissScene(playerWon:)` only increments the count on a win — a losing streak never makes the hive harder.

Four discrete tiers (`BeeDifficultyTier`: `.base`, `.angry`, `.furious`, `.savage`) are keyed off `BeeDifficultyTier.forWins(_:)` — every 3 wins bumps a tier, capped at `.savage` (9+ wins). Each tier scales three things:
- **Speed** (`speedMultiplier`) — 1.0× / 1.4× / 1.8× / 2.2×, applied to every spawned bee via `globalSpeedMultiplier`.
- **Concurrent bees** (`concurrentBonus`) — 0/1/2/3, added on top of the existing within-match 1→3 concurrent ramp in `maxConcurrent` (so `.savage` can have up to 6 bees active at once).
- **Visual tint** (`tintColor`/`tintBlendFactor`) — bees go from untinted at `.base` to amber (0.30 blend) → red (0.50) → near-black deep red (0.70) at `.savage`, applied in `makeBeeSprite()`. This is the "meaner and more upset" characterization the review asked for — same sprite sheet (`Bee_Flying_Animation`), no new art.

`showTierAnnouncementIfNeeded()` fires once per match at `startGame()` for any tier above `.base` — a fading banner ("THE BEES ARE ANGRY!" / "FURIOUS!" / "SAVAGE!" in the tier's tint color) makes the escalation legible instead of a silent stat change, matching the "characterized difficulty" pattern recommended for Billy Badger's rubber-band elsewhere.
